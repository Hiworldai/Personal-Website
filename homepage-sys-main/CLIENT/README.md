...
原因：项目已经升级到 Vite 8，Vite 8 官方要求 Node `^20.19.0 || >=22.12.0`。你当前本机是 Node 24，可以正常运行。
...
GUESTBOOK_TO_EMAIL=1950279740@qq.com
GUESTBOOK_SERVER_PORT=5177
GUESTBOOK_DAILY_LIMIT=3
GUESTBOOK_MESSAGE_MAX_LENGTH=1200
```

注意：`.env` 里是真实密钥，不能上传 GitHub。当前 `.gitignore` 已经忽略 `.env` 和 `.env.*`。
同时 `.gitignore` 里保留了 `!.env.example`，所以示例配置可以上传，真实密钥不会跟着上传。
...
`responsive-report/` 是测试产物，已经写进 `.gitignore`，不需要上传 GitHub。
...
这个项目不是简单的静态个人页，它可以在简历里包装成一个“个人品牌展示 + 交互式作品集 + 留言转邮件系统”的完整小型前后端项目。
...
难点：这里既有全局平滑滚动，也有首页圆圈的自定义滚动，所以代码里临时把 `scrollBehavior` 改成 `auto`，避免两套滚动动画互相打架。
...
当前检查结果：`galleryItems` 当前配置了 32 个展示项，引用到的影集图片和视频文件都能在 `public/gallery/` 中找到。
...
难点：如果只复制 2 份，动画滑到边界时会露空；现在复制 3 份，滚动更连续。
...
重要提醒：你之前把 Resend 密钥发到聊天里了，建议正式上传 GitHub 前去 Resend 后台重新生成一个新 key，并把旧 key 删除。
...
- 影集原图体积有几张较大；当前列表已经使用缩略图，正式上线后还可以继续把原图压缩成 WebP，提高点开大图时的加载速度。
- 现在留言限流存在内存里，服务重启后计数会清空；正式部署可以换成 Redis。
- 如果要把网站部署到 Vercel/Netlify，后端接口需要改成对应平台的 serverless function。
